...
**AI Provider:** Switched to Groq for fast, free developer-tier demos
...
- **ai_helper.py** - Groq integration for translation, analysis, and chat
...
- **Groq API** - AI-powered demo features via the OpenAI-compatible API
...
- **GROQ_API_KEY** - Required for demo AI features
...
The UI checks `success` before processing responses and displays user-friendly error messages when API calls fail or when `GROQ_API_KEY` is not configured.
...
- API key validation before Groq calls
...
## Development Notes
- All AI calls now use Groq-hosted models (`llama-3.1-8b-instant` by default)
- Rate limits and reliability now follow your Groq developer-tier account limits
...
- ✅ Groq API integration wired through `ai_helper.py`
